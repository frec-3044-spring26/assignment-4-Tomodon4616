...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_line() +
  labs(x = "Year", y = "Ice off Day", title = "Ice off Days for Lake Sunapee from 1869 to 2025")
```
...
I expect the sign to be negative as the general trend of the data appears to be going downward.
...
the direction of the slope is downward, this is what I expected to see because you can see an overall negative trend in the data.
...
The R2 value is 20%, so we can say that they are not strongly related.
...
```{r}
library(modelr) 
sunapee <- sunapee |> add_predictions(fit, var = "pred", type = NULL) |>
...
ggplot(data = sunapee) +
  ...
  theme_bw()
```
looking at the histogram, the residuals are not normally distributed due to the slight left skew in the data.
...
about 66 years. I would say that in order for this result to be meaningful, we would have to consider a longer-term view. While ice off day changing by one week in 66 years might not be that meaninful, applying that logic to a longer term of 200 years could show more significant results. 
...
Even though the p-value is significant, the R2 value of .2 shows that there are other factos that need to be considered in order to have a more complete picture of the change in ice off day of the lake. Therefore I would not be too confortable using this model unless more data was provided.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(title = "Model fit of year vs Ice off Day") +
  theme_bw()
```
...
```{r}
ice_off_2040 <- fit$coefficients[2] * 2040 + fit$coefficients[1]
quantiles <- quantile(sunapee$resid, probs = seq(0, 1, .05))
lower <- ice_off_2040 + quantiles["5%"]
upper <- ice_off_2040 + quantiles["95%"]
as.numeric(lower)
as.numeric(upper)
...
the confidence interval is about 34 days wide. Going back to the previous question about using this model to predict when ice off day will change by a week, we can see that this confidence interval basically invalidates any significant guessing power this model has in the short term.
...
```{r}
sunapee1 <- sunapee |> filter(year < 1970) |> mutate(period = 'b')
sunapee2 <- sunapee |> filter(year >= 1970) |> mutate(period = 'a')
comb <- bind_rows(sunapee1, sunapee2)

fit1 = lm(ice_off_doy ~ year, data = sunapee1)
fit2 = lm(ice_off_doy ~ year, data = sunapee2)
summary(fit1)
summary(fit2)

ggplot(comb, aes(x = year, y = ice_off_doy, color = period)) +
  geom_point() +
  geom_abline(slope = fit1$coefficients[2], intercept = fit1$coefficients[1], color = "blue") +
  geom_abline(slope = fit2$coefficients[2], intercept = fit2$coefficients[1], color = "red") +
  labs(title = "Comparison of Ice off Days before and after 1970", x = "year", y = "Ice off Day ") +
  theme_bw()
fit1$coefficients[2]
fit2$coefficients[2]
```
Looking at the plot and the summary of the models, we can see that in recent years, the change in ice_off_day is changing at 3 times the rate as it was in the pastt. The R2 value for recent years is also double that of the earlier years, indicating that the year to ice_off_day trend is more definitive in recent years.
...
github link: <https://github.com/frec-3044-spring26/assignment-4-Tomodon4616>
